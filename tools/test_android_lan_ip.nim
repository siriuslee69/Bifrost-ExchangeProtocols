...
proc shellJoin(args: openArray[string]): string {.role: helper, tag: {tagInterop}.} =
...
proc runChecked(args: openArray[string]): string {.role: orchestrator, tag: {tagInterop}.} =
...
proc localLanIpv4(): string {.role: dataFetcher, tag: {tagNetworkSurface}.} =
...
proc androidApk(root, kind: string): string {.role: helper, tag: {tagInterop}.} =
...
proc androidWifiIpv4(serial: string): string {.role: dataFetcher, tag: {tagNetworkSurface}.} =
...
proc serveExchange(S: ptr HostExchangeState) {.thread, role: orchestrator, tag: {tagNetworkSurface}.} =
...
proc runLanTest(serial: string) {.role: metaOrchestrator, tag: {tagNetworkSurface}.} =
