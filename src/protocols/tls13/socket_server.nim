...
type
  ...
  Tls13SocketResult* {.role: truthState, tag: {tagTls, tagTransport}.} = object
    ok*: bool
    err*: string

proc initTls13SocketSession*(): Tls13SocketSession {.role: truthBuilder,
    tag: {tagTls, tagTransport}.} =
...
proc toStr(b: openArray[byte]): string {.role: helper, tag: {tagTls}.} =
  ## b: bytes to view as a string.
  result = newString(b.len)
  for i, v in b:
    result[i] = char(v)

proc toBytes(s: string): ByteSeq {.role: helper, tag: {tagTls}.} =
...
proc sendAll(sock: var Socket, data: openArray[byte]): bool {.role: dataWriter,
    tag: {tagTls, tagTransport, tagWrite}.} =
...
proc flushOutbound(sock: var Socket, O: Tls13ServerOutput): bool {.
    role: dataWriter, tag: {tagTls, tagTransport, tagWrite}.} =
...
proc readChunk(sock: var Socket, timeoutMs: int): tuple[ok: bool, data: string] {.
    role: dataFetcher, tag: {tagTls, tagTransport, tagRead}.} =
...
proc startTls13ServerSocket*(sock: var Socket, S: var Tls13SocketSession,
    ...
    role: orchestrator, tag: {tagTls, tagTransport, tagCryptoBoundary}.} =
...
proc pumpPlaintext(sock: var Socket, S: var Tls13SocketSession,
    ...
    tag: {tagTls, tagTransport, tagRead}.} =
...
proc tls13SocketClose*(sock: var Socket, S: var Tls13SocketSession) {.
    role: orchestrator, tag: {tagTls, tagTransport}.} =
