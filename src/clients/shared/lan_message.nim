...
proc addU16(A: var ByteSeq, v: uint16) {.role: dataWriter, tag: {tagInterop}.} =
  ## A: output bytes. v: little-endian unsigned value.
  A.add(byte(v and 0xff'u16))
  A.add(byte(v shr 8))

proc addU32(A: var ByteSeq, v: uint32) {.role: dataWriter, tag: {tagInterop}.} =
  ## A: output bytes. v: little-endian unsigned value.
  var
    i: int = 0
  while i < 4:
...
proc addU64(A: var ByteSeq, v: uint64) {.role: dataWriter, tag: {tagInterop}.} =
  ## A: output bytes. v: little-endian unsigned value.
  var
    i: int = 0
  while i < 8:
...
proc readU16(A: openArray[byte], i: int): uint16 {.role: parser, tag: {tagInterop}.} =
  ## A: input bytes. i: first byte offset.
  result = uint16(A[i]) or (uint16(A[i + 1]) shl 8)

proc readU32(A: openArray[byte], i: int): uint32 {.role: parser, tag: {tagInterop}.} =
  ## A: input bytes. i: first byte offset.
  var
    j: int = 0
  while j < 4:
...
proc readU64(A: openArray[byte], i: int): uint64 {.role: parser, tag: {tagInterop}.} =
  ## A: input bytes. i: first byte offset.
  var
    j: int = 0
  while j < 8:
...
proc addText(A: var ByteSeq, s: string) {.role: dataWriter, tag: {tagInterop}.} =
  ## A: output bytes. s: UTF-8 text to append unchanged.
  for c in s:
    A.add(byte(c))

proc readText(A: openArray[byte], i, n: int): string {.role: parser, tag: {tagInterop}.} =
...
proc requireLanText(s, label: string, maxBytes: int) {.role: sanitizer, tag: {tagValidation}.} =
...
proc initLanMessage*(p: LanProtocol, id, name, body: string, sequence: uint64,
    timestampMillis: uint64 = 0'u64): LanMessage {.role: truthBuilder, tag: {tagInterop}.} =
...
proc initLanAck*(source: LanMessage, id, name: string,
    sequence: uint64): LanMessage {.role: truthBuilder, tag: {tagInterop}.} =
...
proc encodeLanMessage*(m: LanMessage): ByteSeq {.role: dataWriter, tag: {tagCodecBoundary, tagInterop}.} =
...
proc decodeLanMessage*(A: openArray[byte]): LanMessage {.role: parser, tag: {tagCodecBoundary, tagInterop}.} =
