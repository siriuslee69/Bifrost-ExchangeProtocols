...
proc pskSecret(seed: byte): ByteSeq =
...
proc newPair(name: string): Pair {.role: configurator.} =
  ...
  result.auth = initAmeCertificateAuthentication(result.root)
...
suite "AME private handshake":
  ...
  test "reciprocal public-key pins authenticate the complete handshake":
    ...
    clientDone = finishAmeHandshake(client, server.state.serverHello,
      initAmePinnedAuthentication(serverPin), clientDesc, clientKey, nowUnix)
  ...
  test "the wrong pin and a mixed-up trust mode both fail closed":
    ...
    clientDone = finishAmeHandshake(client, server.state.serverHello,
      initAmePinnedAuthentication(wrongPin), clientDesc, clientKey, nowUnix)
  ...
  test "tampering anywhere in the server hello fails closed":
    ...
    client = saved
    done = finishAmeHandshake(client, hello, p.auth, p.clientCert,
      p.clientKey, nowUnix)
    check not done.ok
    hello = server.state.serverHello
  ...
  test "an unsupported layout or tier is refused before any key work":
    var
      ...
      server = answerAmeHandshake(client.hello,
        [handshakePath(p.layout, other)], p.auth, p.serverCert, p.serverKey)
    check not server.ok
...
suite "AME handshake transport":
  ...
  test "AM1M proof binds the provisioned identity and transcript":
    ...
    proof = amePskTranscriptProof(auth, apdResponder, transcript)
    check verifyAmePskTranscript(auth, apdResponder, transcript, proof)
    altered = transcript & @[byte 4]
    check not verifyAmePskTranscript(auth, apdResponder, altered, proof)
    ## The other end's proof over the same bytes is a different tag, so one
    ## can never be replayed in place of the other.
    check not verifyAmePskTranscript(auth, apdInitiator, transcript, proof)
    check amePskTranscriptProof(auth, apdInitiator, transcript) != proof
    auth.pskId = "peer-b"
    check not verifyAmePskTranscript(auth, apdResponder, transcript, proof)
    clearAmeAuthentication(auth)
    check auth.psk.len == 0


  ## The test that was missing. Everything above only exercised the proof
  ...
  test "a complete AM1M handshake runs with no certificate on either side":
    ...
    ## The exchange proof key is derived, shared, and is not the secret.
    check clientDone.auth.exchangeAuthenticationKey.len == 32
  ...
  test "a wrong shared secret, a wrong name, and a mode mismatch fail closed":
    var
      auth: AmeAuthentication = initAmePskAuthentication("site-a",
        pskSecret(1))
      wrongSecret: AmeAuthentication = initAmePskAuthentication("site-a",
        pskSecret(9))
      wrongName: AmeAuthentication = initAmePskAuthentication("site-b",
        pskSecret(1))
      layout: AmeSuiteLayout = handshakeLayout()
      tier: AmeMaskTier = handshakeTier(layout)
    ...
    check server.ok
    ## A peer holding a different secret cannot open the block at all -- the
    ## binder went into the key schedule, so this fails before any proof.
    client = saved
    done = finishAmeHandshake(client, server.state.serverHello, wrongSecret)
    check not done.ok
    check done.err == "server identity block failed authentication"
    ## The right secret under the wrong name opens nothing either.
    client = saved
  ...
  test "AM1M rotates an epoch with no signature keys to sign with":
    ...
    a = initAmeSession(clientDone.auth, handshakePath(layout, tier), 74'u64,
      peerTrust = clientDone.peerTrust)
    b = initAmeSession(serverDone.auth, handshakePath(layout, tier), 74'u64,
      peerTrust = serverDone.peerTrust)
    ## An offer and a reply that carry MAC proofs instead of signatures.
    offer = beginAmeSessionExchange(a, initAmeExchangeRequest(layout.kems,
      tier, tier.masks.kem))
    check offer.signatures.len == 1
    ...
    check a.auth.current.epochId == 2'u32
    check b.auth.current.epochId == 2'u32
    ## And the rotated epoch still carries traffic both ways.
    frame = sealAmeTcpFrame(a, @[byte 5, 6, 7])
    opened = openAmeTcpFrame(b, frame)
    check opened.ok
    check opened.packet.payload == @[byte 5, 6, 7]
    ## A tampered proof is refused rather than ignored.
    offer = beginAmeSessionExchange(a, initAmeExchangeRequest(layout.kems,
      tier, tier.masks.kem))
    offer.signatures[0][0] = offer.signatures[0][0] xor 0xFF'u8
    expect ValueError:
