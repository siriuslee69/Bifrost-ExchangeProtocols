...
suite "FOMKE":
  ...
  ## What happens when a message is not merely late but gone. Without a way
  ## to give up on it, the two rules that keep the ratchet honest -- refuse a
  ## gap wider than the budget, refuse a rekey while keys are outstanding --
  ## between them leave a lossy link with no way forward at all.
  # {.testKind: tkRegression.}
  test "messages that never arrive can be given up on, and then a rekey runs":
    var
      initial: AmeExchangeState = initialExchangeState()
      candidate: AmeExchangeState = upgradedExchangeState()
      request: AmeExchangeRequest = initAmeExchangeRequest(fomkeKems,
      ...
      bob: FomkeState = initFomkeFromAme(initial, fomkeLayout(),
        fomkeInitialTier(), frResponder)
      lost: FomkeMessage
      arrived: FomkeMessage
      opened: FomkeOpenResult
      dropped: int = 0
      aliceCommit: FomkeUpgradeCommit
      bobCommit: FomkeUpgradeCommit
    lost = sealFomkeMessage(alice, @[byte 1])
    arrived = sealFomkeMessage(alice, @[byte 2])
    discard lost
    opened = openFomkeMessage(bob, arrived)
    check opened.ok
    check fomkeSkippedMessages(bob) == 1
    ## While that key is outstanding the rekey is refused, exactly as before.
    expect ValueError:
      discard prepareFomkeUpgrade(bob, 12'u32, 2'u32, request, candidate)
    ## Giving up on it says how many were given up on, and clears the way.
    dropped = discardFomkeSkipped(bob)
    check dropped == 1
    check fomkeSkippedMessages(bob) == 0
    bobCommit = prepareFomkeUpgrade(bob, 12'u32, 2'u32, request, candidate)
    aliceCommit = prepareFomkeUpgrade(alice, 12'u32, 2'u32, request, candidate)
    check fomkeUpgradeCommitsEqual(aliceCommit, bobCommit)
    confirmFomkeUpgrade(bob, aliceCommit)
    confirmFomkeUpgrade(alice, bobCommit)
    check bob.epoch == 2'u32
    ## And the message that was given up on stays shut for good, even if the
    ## network does eventually deliver it.
