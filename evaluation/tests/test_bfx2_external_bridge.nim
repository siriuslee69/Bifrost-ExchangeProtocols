...
suite "BFX2 External Bridge":
  ...
  test "external bridge vector is stable":
    var
      ...
      enc: tuple[ok: bool, packet: ByteSeq, err: string]
      dec: ExternalDecodeResult
    hexPath = joinPath(getCurrentDir(), "evaluation", "tests", "vectors", "bfx2_external", "ermine_schema_410_v2.hex")
    jsonPath = joinPath(getCurrentDir(), "evaluation", "tests", "vectors", "bfx2_external", "ermine_schema_410_v2.json")
