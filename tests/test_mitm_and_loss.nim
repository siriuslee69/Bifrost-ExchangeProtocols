  ...
  test "no key material appears in any of the four records":
    var
      ...
      server = answerAmeHandshake(client.hello,
        [initAmeTierPath(layout, [tier])], serverCert, serverKey)
    ...
    for b in encodeAmeClientFinish(clientDone.finish):
      captured.add(b)
    serverDone = acceptAmeHandshake(server.state, clientDone.finish, root,
      nowUnix)
    check serverDone.ok
  ...
  test "a MITM cannot swap in his own identity":
    var
      ...
      attacker = answerAmeHandshake(client.hello,
        [initAmeTierPath(layout, [tier])], rogueCert, rogueKey)
      clientDone: AmeHandshakeResult
    check attacker.ok
    ...
    clientDone = finishAmeHandshake(client, attacker.state.serverHello, root,
      clientCert, clientKey, nowUnix)
