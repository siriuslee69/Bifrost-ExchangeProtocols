...
suite "Bifrost task contract":
  ...
  test "repo hygiene finds root helpers and nested source build trees":
    ...
    writeFile(root / "tests" / "test_crypto_suites.exe", "bin")
    ...
    while i < findings.len:
      ...
      of "src/libbifrost_exchange_protocols.so":
        sawSharedLib = true
      of "tests/test_crypto_suites.exe":
        sawTestExe = true
  ...
  test "flake check wires TLS verification through Nix":
    ...
    check tlsCheckText.find("""tests/test_transport_ops.nim""") >= 0
  ...
  test "benchmark harness stays committed and wired":
    ...
    check content.find("""runNim("c", "tools/bench_protocols.nim", @["-d:release", "-r"])""") >= 0
    check benchDoc.find("does not yet ship a committed Otter benchmark harness") < 0
    check benchDoc.find("tools/bench_protocols.nim") >= 0
    check benchDoc.find("nim c -d:release -r tools/bench_protocols.nim") < 0
    check benchDoc.find("--out:build/tools/bench_protocols") >= 0
    check readmeText.find("`nimble benchmarks`") >= 0
    check readmeText.find("`--out:build/tools/...`") >= 0
...
test "repo scan has no stale project-name or alias references":
  ...
  test "release docs describe closed readiness gaps as closed":
    ...
    findingsDoc = readFile("findings.md")
    readmeText = readFile("README.md")
    contributingText = readFile("CONTRIBUTING.md")
    testsDoc = readFile("docs/tests.md")
    ...
    check findingsDoc.find("remaining risks were in the Android harness and repo hygiene") < 0
    check findingsDoc.find("trust-all TLS verification and bundled repo private key") < 0
    check findingsDoc.find("missing `docs/`, `CONTRIBUTING.md`, and committed license text") < 0
