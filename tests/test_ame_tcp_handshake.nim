...
proc runAmeTcpServer(a: AmeTcpServerArgs) {.thread.} =
  ...
  config = initAmeResponderPolicy([initAmeTierPath(layout, [tier])],
    ...
    if a.pinned: atmPinnedPeerKey else: atmAuthorityCertificate,
    a.requireCookie)
  config.root = initAmeAuthorityRoot(authority)
  if a.pinned:
    config.expectedPeer = pinnedPeerIdentity(clientKey)
  defer:
    listener.close()
...
proc clientConfig(pinned: bool, authorityTag: int): AmeInitiatorPolicy =
  ## pinned/authorityTag: initiator policy matching one server variant.
  var
    layout: AmeSuiteLayout = tcpLayout()
    clientKey: AmeIdentityKey = identityFor("tcp-client", 2)
    serverKey: AmeIdentityKey = identityFor("tcp-server", 3)
  ...
  if pinned:
    result = initAmeInitiatorPolicy(layout, tcpTier(layout),
      ...
      atmPinnedPeerKey)
    result.expectedPeer = pinnedPeerIdentity(serverKey)
  else:
    result = initAmeInitiatorPolicy(layout, tcpTier(layout),
      issueAmeIdentityCertificate(authority, clientKey, 11'u64, validFrom,
        validUntil), clientKey)
    result.root = initAmeAuthorityRoot(authority)
  ...
  test "a certified handshake survives the cookie retry and carries data":
    var
      ...
      args: AmeTcpServerArgs = AmeTcpServerArgs(port: echoPort,
        requireCookie: true, pinned: false, authorityTag: 1)
      sock: Socket
      outcome: AmeHandshakeOutcome
    ...
    outcome = ameTcpClientHandshake(sock, clientConfig(false, 1), 7'u64,
      nowUnix, 20000)
    check outcome.err == ""
    check outcome.ok
  ...
  test "reciprocal pins authenticate the same driver":
    var
      ...
      args: AmeTcpServerArgs = AmeTcpServerArgs(port: pinnedPort,
        requireCookie: false, pinned: true, authorityTag: 1)
      sock: Socket
      outcome: AmeHandshakeOutcome
    ...
    outcome = ameTcpClientHandshake(sock, clientConfig(true, 1), 8'u64,
      nowUnix, 20000)
    check outcome.err == ""
    check outcome.ok
  ...
  test "a client trusting the wrong authority is refused on the wire":
    var
      ...
      args: AmeTcpServerArgs = AmeTcpServerArgs(port: rejectPort,
        requireCookie: false, pinned: false, authorityTag: 1)
      sock: Socket
      outcome: AmeHandshakeOutcome
    serverReport = default(AmeTcpServerReport)
    serverReady.store(false)
    ...
    outcome = ameTcpClientHandshake(sock, clientConfig(false, 4), 9'u64,
      nowUnix, 20000)
